...
function toggleSpotColorOptions(method) {
    ...
    if (method !== 'uv-offset') {
        // Reset UV options when not in UV mode
        const uvCheckboxes = uvOptionsDiv.querySelectorAll('input[type="checkbox"]');
        uvCheckboxes.forEach(checkbox => checkbox.checked = false);
    }

    // 显示对应的选项
    if (method === 'offset') {
        spotColorDiv.classList.remove('hidden');
    } else if (method === 'uv-offset') {
...
function updateQuote() {
    ...
    } else if (printingMethod === 'uv-offset') {
        baseFactor *= 1.3; // UV offset has higher base cost
        
        // 检查是否使用了白墨
        if (document.querySelector('input[name="uv-white-ink"]:checked')) {
            additionalCost += 0.3 * quantity * 8; // 白墨成本
        }
        
        // 检查是否使用了逆向UV
        if (document.querySelector('input[name="uv-reverse"]:checked')) {
            additionalCost += 0.4 * quantity * 10; // 逆向UV成本
        }
    }
