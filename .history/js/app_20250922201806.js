...
function handleMaterialChange() {
    ...
    const thicknesses = Object.keys(materialInfo.thicknesses);
    const html = `
        <div>
            <label class="block text-sm font-medium text-slate-700 mb-2">纸张厚度</label>
            <div class="flex flex-wrap gap-2" id="thickness-options">
                ${thicknesses.map((thickness, index) => `
                    <label class="border rounded-md px-3 py-2 cursor-pointer has-[:checked]:bg-blue-50 has-[:checked]:border-blue-500 text-sm hover:shadow-sm transition-all">
                        <input type="radio" name="thickness" value="${thickness}"
                            onchange="handleThicknessChange('${selectedMaterial}', this.value)"
                            class="sr-only"
                            ${index === 0 ? 'checked' : ''}>
                        ${thickness}
                    </label>
        ...
        <div id="grammage-selection-container" class="mt-4"></div>
    `;

    grammageContainer.innerHTML = html;

    // 默认选中第一个厚度并渲染克重
    const defaultThickness = thicknesses[0];
    handleThicknessChange(selectedMaterial, defaultThickness);
    updateQuote();
}

function handleThicknessChange(materialType, thickness) {
    ...
    grammages.forEach(g => {
        html += `
            <label class="border rounded-md px-3 py-2 cursor-pointer has-[:checked]:bg-blue-50 has-[:checked]:border-blue-500 text-sm hover:shadow-sm transition-all">
                <input type="radio" name="grammage" value="${g.value}"
                    data-price-factor="${g.factor}"
