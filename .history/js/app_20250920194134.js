...
function renderOrdersPage(filterStatus = 'all', searchQuery = '') {
    ...
    if (filteredOrders.length === 0) {
        let message = '暂无相关订单';
        if (searchQuery) message = `未找到包含 "${searchQuery}" 的订单`;
        ...
        container.innerHTML = filteredOrders.map(order => {
            ...
            const status = orderStates.find(s => s.id === order.statusId) || { name: order.status };
            const expectedDelivery = order.statusId === 'placed' ? '预计 7-10 天' : 
                                   order.statusId === 'processing' ? '预计 5-7 天' : 
                                   order.statusId === 'shipped' ? '预计 2-3 天' : '已完成';
            const canCancel = ['placed', 'processing'].includes(order.statusId);
            const canReturn = order.statusId === 'shipped' || order.statusId === 'completed';
            const actionButtons = [];
            if (canCancel) {
                actionButtons.push(`<button onclick="openCancelOrderModal('${order.id}')" class="text-red-600 hover:text-red-800 bg-red-50 px-3 py-1 rounded-md text-xs font-semibold border border-transparent hover:bg-red-50 transition-colors">取消订单</button>`);
            }
            if (order.statusId === 'shipped') {
                actionButtons.push(`<button onclick="openLogisticsModal('${order.id}')" class="text-blue-600 hover:text-blue-800 bg-blue-50 px-3 py-1 rounded-md text-xs font-semibold border border-transparent hover:bg-blue-50 transition-colors">查看物流</button>`);
            }
            if (canReturn) {
                actionButtons.push(`<button onclick="openReturnOrderModal('${order.id}')" class="text-orange-600 hover:text-orange-800 bg-orange-50 px-3 py-1 rounded-md text-xs font-semibold border border-transparent hover:bg-orange-50 transition-colors">申请退款</button>`);
            }
            actionButtons.push(`<button onclick="showOrderDetailInModal('${order.id}')" class="text-blue-600 hover:text-blue-800 bg-blue-50 px-3 py-1 rounded-md text-xs font-semibold border border-transparent hover:bg-blue-50 transition-colors">订单详情</button>`);
            actionButtons.push(`<button onclick="contactCustomerService('${order.id}')" class="text-gray-600 hover:text-gray-800 px-3 py-1 rounded-md text-xs font-semibold transition-colors">联系客服</button>`);

            return `
            <div class="order-card border border-slate-200 rounded-lg mb-6 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                <div class="bg-slate-50 px-6 py-3 flex flex-col md:flex-row justify-between items-start md:items-center text-sm">
                    <div class="md:flex-1">
                        <div class="flex justify-between md:justify-start items-start md:items-center space-y-2 md:space-y-0 md:flex-row md:space-x-4">
                            <div class="flex items-center space-x-2">
                                <span class="font-semibold">订单号:</span> 
                                <span class="text-slate-600">#${order.id}</span>
                            </div>
                            <div class="flex-1">
                                <span class="font-semibold">下单时间:</span> 
                                <span class="text-slate-600">${order.date}</span>
                            </div>
                            <div class="flex ml-0 md:ml-4">
                                <span class="font-semibold px-2 py-1 rounded text-xs font-semibold bg-${status.id === 'processing' ? 'yellow' : status.id === 'shipped' ? 'blue' : status.id === 'completed' ? 'green' : 'gray'}-100 text-${status.id === 'processing' ? 'yellow' : status.id === 'shipped' ? 'blue' : status.id === 'completed' ? 'green' : 'gray'}-800">${order.status}</span>
                            </div>
                        </div>
                        <div class="flex ml-0 md:ml-0 mt-2 md:mt-0 md:flex-col lg:flex-row space-y-2 md:space-y-0 md:space-x-4 text-xs text-slate-500">
                            <span>预计到货: ${expectedDelivery}</span>
                            <span class="hidden md:inline"> | 共 ${order.items.length} 件商品 | 实付 <span class="font-semibold text-slate-900">¥${order.total.toFixed(2)}</span></span>
                            <span class="md:hidden">¥${order.total.toFixed(2)}</span>
                        </div>
                    </div>
                </div>
                <div class="p-6">
                    <div class="flex flex-col md:flex-row items-start space-y-4 md:space-y-0 md:space-x-4">
                        <img src="${firstItem.imageUrl}" alt="${firstItem.name}" class="w-24 h-24 rounded-md object-cover flex-shrink-0" onclick="showOrderDetailInModal('${order.id}', true)">
                        <div class="flex flex-col md:flex-grow lg:min-w-0">
                            <h4 class="font-semibold text-lg">${firstItem.name}</h4>
                            <p class="text-sm text-slate-500">${firstItem.specs || '定制规格'}</p>
                            <div class="mt-2 md:hidden">
                                <p class="text-xs text-slate-500">数量: ${order.items.reduce((sum, i) => sum + i.quantity, 0)} 件 | <span class="font-semibold text-slate-900">¥${order.total.toFixed(2)}</span></p>
                            </div>
                            ${order.items.length > 1 ? `<button onclick="showOrderDetailInModal('${order.id}')" class="text-blue-600 hover:underline text-sm mt-1 md:hidden">查看完整订单</button>` : ''}
                        </div>
                        <div class="md:flex-shrink-0 md:text-right">
                            <p class="text-lg font-bold text-slate-900 md:block hidden">¥${order.total.toFixed(2)}</p>
                            <p class="text-sm text-slate-500 md:block hidden">全款支付</p>
            ...
            `;
        }).filter((html, index) => filteredOrders[index] ? true : false).map(html => html).join('');
    }
...
function handleOrderSearch(query) {
    ...
    searchTimeout = setTimeout(() => {
        renderOrdersPage(currentOrderFilter, query.trim());
    }, 300);
}
...
function openCancelOrderModal(orderId) {
    ...
    if (modal) {
        ...
        if (confirm(`确认取消订单 #${order.id}？ (退款到原支付方式)`) {
            order.statusId = 'cancelled';
            order.status = '已取消';
            localStorage.setItem('orders', JSON.stringify(orders));
            renderOrdersPage(currentOrderFilter);
            showNotification('订单已取消，款项将在1-3日内退回', 'success');
        }
    }
}
...
function confirmCancelOrder() {
    ...
    // 关闭模态并更新
    document.getElementById('cancel-order-modal').classList.add('hidden');
    showNotification('取消请求已提交，款项将在审核后退回', 'success');
    // 实际状态更新在 showUserCenterView(order-detail-view) 中处理
}
...
function openLogisticsModal(orderId) {
    const order = orders.find(o => o.id === orderId);
    if (!order) return;
    alert(`订单 #${order.id} 物流信息：预计${Math.floor(Math.random() * 3 + 2)}天到货。实际需集成物流 API。`);
}
...
function openReturnOrderModal(orderId) {
    const order = orders.find(o => o.id === orderId);
    if (!order) return;
    if (confirm(`申请退款订单 #${order.id}？ (需提供原因)`) {
        // 状态更新
        order.statusId = 'refund-pending';
        order.status = '退款申请中';
        localStorage.setItem('orders', JSON.stringify(orders));
...
function contactCustomerService(orderId) {
    const order = orders.find(o => o.id === orderId);
    alert(`客服联系：订单 #${order.id}，电话 400-888-8888 或邮件 service@iboxify.com`);
}
